...
public class Helpers {
	...
	// private static final String PREF_FILESIZE_KEY = "FILE_SIZE_LENGTH";

	// private static final long FILESIZE_NO_RECORD = -1;
	...
	static public String getMBFromBytes(long overallTotal) {
		if (overallTotal == 0) {
			if (Constants.LOGVV) {
				Log.e(Constants.TAG, "Notification called when total is zero");
			}
		...
		return String.format("%.2f", (float) overallTotal / (1024.0f * 1024.0f)) + "MB";
	}
	 ...
	 */
	public static String getExpansionAPKFileNameZip(Context c, boolean mainFile, int versionCode) {
	...
	static public String generateSaveFileNameTMP(Context c) {
		String path = generateSaveFileName(c, getExpansionAPKFileName(c, true, Helpers.getVersionCode(c))) + TEMP_EXT;
		return path;
	}

	static public String generateSaveFileNameFullPath(Context c) {
		String path = getSaveFilePath(c) + File.separator + getExpansionAPKFileName(c, true, getVersionCode(c));
		return path;
	}
	...
	static public String getSaveFilePathNoPackageName(Context c) {
		File root = Environment.getExternalStorageDirectory();
		String path = root.toString() + Constants.EXP_PATH;
		return path;
	}
	...
	static public boolean doesFileExistByFileSize(Context c, long fileSize, boolean deleteExistFailFile)
			throws IOException {
		// get file size from preferences

		File fileForNewFile = new File(
				Helpers.generateSaveFileName(c, getExpansionAPKFileName(c, true, getVersionCode(c))));

		// File fileForNewFileTMP = new File(
		// Helpers.generateSaveFileName(c, getExpansionAPKFileName(c, true,
		// getVersionCode(c))) + ".tmp");

		if (fileForNewFile.exists()) {
			if (fileForNewFile.length() == fileSize) {
				return true;
			} else if (deleteExistFailFile)
				fileForNewFile.delete();
		}
		// else if (fileForNewFileTMP.exists()) {
		// return false;
		// long fileForNewFileTMPLength = fileForNewFileTMP.length();
		// if (fileForNewFileTMP.length() == fileSize) {
		// return true;
		// } else if (deleteExistFailFile)
		// fileForNewFileTMP.delete();
		// }
		return false;
	}

	static public boolean doesFileExistByFileSizeTMP(Context c, long fileSize, boolean deleteExistFailFile)
			throws IOException {
		File fileForNewFileTMP = new File(
				Helpers.generateSaveFileName(c, getExpansionAPKFileName(c, true, getVersionCode(c))) + TEMP_EXT);

		if (fileForNewFileTMP.exists()) {
			if (fileForNewFileTMP.length() == fileSize) {
				return true;
			} else if (deleteExistFailFile)
				fileForNewFileTMP.delete();
	...
	// public static long getFileSize(Context c, String url) throws IOException
	// {
	// long fileSize = getFileSizePref(c);
	// if (fileSize == FILESIZE_NO_RECORD) {
	// URL urlObj = new URL(url);
	// HttpURLConnection connection = (HttpURLConnection)
	// urlObj.openConnection();
	// fileSize = connection.getContentLength();
	// saveFileSize(c, fileSize);
	// connection.disconnect();
	// }
	// return fileSize;
	// }

	// private static long getFileSizePref(Context context) {
	// SharedPreferences preferences =
	// PreferenceManager.getDefaultSharedPreferences(context);
	// long downID = preferences.getLong(getPrefFileSizeKey(context),
	// FILESIZE_NO_RECORD);
	// return downID;
	// }
	//
	// private static void saveFileSize(Context context, long filesize) {
	// SharedPreferences preferences =
	// PreferenceManager.getDefaultSharedPreferences(context);
	// SharedPreferences.Editor editor = preferences.edit();
	// editor.putLong(getPrefFileSizeKey(context), filesize);
	// editor.apply();
	// }

	// private static String getPrefFileSizeKey(Context context) {
	// // unique pref_file_size_key for when user update new version
	// return PREF_FILESIZE_KEY + "_" + getVersionCode(context);
	// }
	...
	public static int getVersionCode(Context context)// 鐚菠璺皳閿熻В锟芥拋褉锟藉叓鍠⑽垫拃顑炪儻鎷疯�?(姘撻垾鐘拷纬鈹㈡崠顭嬵�?顖楋拷鐘嗐儖鍠☆個銉嫹璺�)
	{
		try {
